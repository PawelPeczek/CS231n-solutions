...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        z1, cacheZ1 = affine_forward(X, self.params['W1'], self.params['b1'])
        a1, cacheA1 = relu_forward(z1)
        z2, cacheZ2 = affine_forward(a1, self.params['W2'], self.params['b2'])
        scores, cacheA2 = relu_forward(z2)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        loss = loss + self.reg * \
               0.5 * (np.sum(self.params['W1'] * self.params['W1']) + np.sum(self.params['W2'] * self.params['W2']))
        dA2 = relu_backward(dL, cacheA2)
        dZ2, dW2, db2 = affine_backward(dA2, cacheZ2)
        dW2 = dW2 + self.reg * self.params['W2']
        dA1 = relu_backward(dZ2, cacheA1)
        _, dW1, db1 = affine_backward(dA1, cacheZ1)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers + 1):
            L2Sum += np.sum(self.params['W' + str(i)] * self.params['W' + str(i)])
        loss += self.reg * 0.5 * L2Sum

        dZ, dW, db = affine_backward(dL, storage['z' + str(self.num_layers)][1])
